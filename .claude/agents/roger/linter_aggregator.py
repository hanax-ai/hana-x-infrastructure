...
class LinterAggregator:
    ...
    def _add_issue(self, issue: Issue) -> bool:
        ...
        fingerprint = self._generate_fingerprint(
            issue.file,
            issue.line,
            f"{issue.source}:{issue.message[:50]}"
        )
    ...
    def _run_parallel(self):
        ...
        with ThreadPoolExecutor(max_workers=6) as executor:
            ...
            futures = {
                ...
                for name, func in linter_functions
            }
            ...
            if result.stdout:
                ...
                for item in data.get('results', []):
                    ...
                    issue = Issue(
                        ...
                        priority=severity_map.get(item.get('issue_severity', 'LOW'), Priority.MEDIUM),
    ...
    def _run_pylint(self):
        ...
        try:
            result = subprocess.run(
                [LINTER_PATHS['pylint'], str(self.path), '--output-format=json', '--exit-zero'],
                capture_output=True,
                text=True,
                timeout=120
            )
            ...
            if result.stdout:
                ...
                for item in data:
                    ...
                    issue = Issue(
                        ...
                        details=f"{item.get('symbol', 'unknown')} ({item.get('message-id', 'unknown')})",
                    ...
                    for func_data in functions:
                        ...
                        if complexity > 10:  # Threshold: 10
                            ...
                            issue = Issue(
                                ...
                                message=f"Function '{func_data.get('name', 'unknown')}' has complexity {complexity} (target: ≤10)",
    ...
    def _run_pytest(self):
        ...
        try:
            ...
            has_tests = (
                (self.path / 'tests').exists() or
                (self.path / 'test').exists() or
                (self.path / 'pytest.ini').exists() or
                (self.path / 'pyproject.toml').exists()
            )
            ...
            result = subprocess.run(
                [LINTER_PATHS['pytest'], '--cov=.', '--cov-report=json', '--quiet', '--tb=no'],
                capture_output=True,
                text=True,
                timeout=300,
                cwd=self.path
    ...
    def _aggregate(self, execution_time: float) -> AggregatedResult:
        ...
        self.issues.sort(key=lambda x: (priority_order.get(x.priority, 5), x.file, x.line or 0))
        ...
        # Generate summary
        summary = self._generate_summary(len(self.issues), critical, high, medium, low, info)
    ...
    def _generate_summary(self, total: int, critical: int, high: int, medium: int, low: int, info: int) -> str:
